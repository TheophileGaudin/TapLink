...
//                isKeyboardVisible: ${keyboardContainer.visibility == View.VISIBLE}
//                Position: ($x, $y)
//            """.trimIndent())

                // Get toggle bar position for reference in anchored mode
                val toggleBarLocation = IntArray(2)
                leftToggleBar.getLocationOnScreen(toggleBarLocation)

                // Adjust coordinates for anchored mode
                ...
                //Log.d("CursorDebug", "Adjusted position: ($adjustedX, $adjustedY)")
...
//    """.trimIndent())
    ...
    override fun onInterceptTouchEvent(ev: MotionEvent): Boolean {
        ...
        if (isAnchored) {
            ...
            Log.d("TouchDebug", """
        DualWebViewGroup onInterceptTouchEvent:
        Action: ${ev.action}
    ...
    """.trimIndent())
        ...
        if (keyboardContainer.visibility == View.VISIBLE) {
            if (isAnchored) {
                ...
                val keyboardX = adjustedX
                val keyboardY = 480 - adjustedY

                // If touch is within keyboard bounds, send event to keyboard
                val isInKeyboardBounds = keyboardY >= 0 &&
                        keyboardY <= keyboardContainer.height &&
                        keyboardX >= 0 &&
                        keyboardX <= keyboardContainer.width

                Log.d("TouchDebug", """
                Keyboard bounds check:
                location: ${location[0]}, ${location[1]}
                keyboardX: $keyboardX
                keyboardY: $keyboardY
                keyboard height: ${keyboardContainer.height}
                keyboard width: ${keyboardContainer.width}
                isInKeyboardBounds: $isInKeyboardBounds
            """.trimIndent())

                if (isInKeyboardBounds) {
                    Log.d("TouchDebug", "Intercepting touch for keyboard in anchored mode")
                    customKeyboard?.dispatchTouchEvent(ev)
                    return true  // Only intercept if we're in keyboard bounds
                }
                return false  // Pass through if not in keyboard bounds
            } else {
                // Regular non-anchored keyboard behavior
                when (ev.action) {
                ...
                return true  // Intercept for non-anchored keyboard behavior
            }
        }

        // Don't intercept if keyboard isn't visible
        return false
    }
    ...
    """.trimIndent())
        ...
        if (keyboardContainer.visibility == View.VISIBLE) {
            customKeyboard?.let { keyboard ->
                return keyboard.dispatchTouchEvent(event)
            }
        }
    ...
    private fun disableTextHandles(editText: EditText) {
        try {
            ...
            editor?.javaClass?.getDeclaredField("mSelectHandleLeft")?.apply {
                isAccessible = true
                set(editor, null)
            }
            editor?.javaClass?.getDeclaredField("mSelectHandleRight")?.apply {
                isAccessible = true
                set(editor, null)
            }
            editor?.javaClass?.getDeclaredField("mSelectHandleCenter")?.apply {
                isAccessible = true
                set(editor, null)
            }
        } catch (e: Exception) {
            Log.e("DualWebViewGroup", "Error disabling text handles", e)
        }
    }
