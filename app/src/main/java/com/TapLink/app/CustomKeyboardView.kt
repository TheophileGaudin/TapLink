...
class CustomKeyboardView @JvmOverloads constructor(
    ...
    private val clickTimeout = 200L // milliseconds
    ...
    fun updateKeyFocus() {
        if (isAnchoredMode) {
            ...
            // Get parent DualWebViewGroup for cursor position
            var parent = parent
            while (parent != null) {
                if (parent is DualWebViewGroup) {
                    // Get cursor position relative to keyboard
                    val location = IntArray(2)
                    getLocationOnScreen(location)
                    val localX = parent.lastCursorX - location[0]
                    val localY = parent.lastCursorY - location[1]

                    // Find and highlight key under cursor
                    getKeyAtPosition(localX, localY)?.let { button ->
//                        button.setBackgroundColor(Color.BLUE)
//                        button.setTextColor(Color.WHITE)
                        //Log.d("KeyboardDebug", "Key under cursor: ${button.text}")
                    }
                    break
                }
            ...
            button.setBackgroundColor(Color.DKGRAY)
            button.setTextColor(Color.WHITE)
            button.invalidate() // Force immediate redraw
            }
        ...
        // Sync with parent if not currently syncing to avoid recursion
        if (!isSyncing) {
            syncWithParent()
        }
    }
    ...
    fun handleDrag(x: Float, action: Int) {
        Log.d("FlingDebug", "handleDrag called with x=$x, action=$action")

        if (isAnchoredMode) {
            // Ignore drag events in anchored mode
            Log.d("FlingDebug", "Ignoring drag in anchored mode")
            return
        }

        when (action) {
            MotionEvent.ACTION_DOWN -> {
                // Reset state on new touch-down
                resetDragState()
                startX = x
                lastX = x
                ...
                firstMove = true
                isDragging = false
                Log.d("FlingDebug", "Touch down at: $x")
            }
            MotionEvent.ACTION_MOVE -> {
                if (firstMove) {
                    // First move after touch-down
                    firstMove = false
                    Log.d("FlingDebug", "First move at: $x")
                }

                val deltaX = x - lastX
                val totalMove = abs(x - startX)

                Log.d("FlingDebug", "Move detected - deltaX: $deltaX, totalMove: $totalMove")

                // Determine if this should be a drag
                if (totalMove > clickThreshold && !isDragging) {
                    isDragging = true
                    Log.d("FlingDebug", "Started dragging")
                }

                // If we're dragging, handle the movement
                if (isDragging) {
                    accumulatedX += deltaX
                    Log.d("FlingDebug", "Dragging - accumulatedX: $accumulatedX")

                    if (accumulatedX >= movementThreshold) {
                        // Forward fling (positive velocity) - move horizontally
                        Log.d("FlingDebug", "Forward fling - Moving right")
                        moveFocusRight()
                        updateKeyFocus()
                        syncWithParent()
                        accumulatedX = 0f
                    } else if (accumulatedX <= -movementThreshold) {
                        // Backward fling (negative velocity) - move vertically
                        Log.d("FlingDebug", "Backward fling - Moving down")
                        moveFocusDown()
                        updateKeyFocus()
                        syncWithParent()
                        accumulatedX = 0f
            ...
            MotionEvent.ACTION_UP -> {
                // Reset state on touch-up
                Log.d("FlingDebug", "Touch up at: $x")
                resetDragState()
            }
        }
    }
...
//            Total move: $totalMove px
//            Was dragging: $isDragging
//            First move: $firstMove
//        """.trimIndent())

        if (!isDragging && touchDuration < clickTimeout && totalMove < clickThreshold) {
            Log.d("TouchDebug", "Processing as tap")
            handleTap()
        }
    ...
    private fun getKeyAtPosition(x: Float, y: Float): Button? {
        ...
        for (i in 0 until keyboard.childCount) {
            ...
            for (j in 0 until row.childCount) {
                ...
                val buttonX = buttonLocation[0] - myLocation[0]
                val buttonY = buttonLocation[1] - myLocation[1]

//                Log.d("KeyboardDebug", """
//                Checking button: ${button.text}
//                Button bounds: ($buttonX, $buttonY) to (${buttonX + button.width}, ${buttonY + button.height})
//            """.trimIndent())
...
//        Log.d("TouchDebug", """
//            CustomKeyboardView: Touch Event
//            Action: ${getActionString(event.action)}
//            X: ${event.x}, Y: ${event.y}
//            Source: ${event.source}
...
//        """.trimIndent())
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                //Log.d("TouchDebug", "Touch DOWN detected")
                touchStartTime = System.currentTimeMillis()
                lastTouchX = event.x
                lastTouchY = event.y
                isDragging = false
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                //Log.d("TouchDebug", "Touch MOVE - Delta X: $deltaX")

                if (abs(deltaX) > clickThreshold) {
                    isDragging = true
                    accumulatedX += deltaX
            ...
            MotionEvent.ACTION_UP -> {
                val touchDuration = System.currentTimeMillis() - touchStartTime
                val totalMove = abs(event.x - lastTouchX)

//                Log.d("TouchDebug", """
//                    Touch UP
//                    Duration: $touchDuration ms
//                    Movement: $totalMove px
//                    Was Dragging: $isDragging
//                """.trimIndent())
                ...
                if (!isDragging && touchDuration < clickTimeout && totalMove < clickThreshold) {
                    if(!isAnchoredMode) {
                        getFocusedKey()?.let { button ->
                            Log.d("TouchDebug", "Processing click from TouchEvent on: ${button.text}")
                            handleButtonClick(button)
                        }
                    }
                    else {
